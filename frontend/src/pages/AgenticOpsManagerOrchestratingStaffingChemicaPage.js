...
// Feature: Agentic ops manager orchestrating staffing, chemicals, and pricing for each location based on weather
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticOpsManagerOrchestratingStaffingChemicaPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-ops-manager-orchestrating-staffing-chemica/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic ops manager orchestrating staffing, chemicals, and pricing for each location based on weather</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-ops-manager-orchestrating-staffing-chemica/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
