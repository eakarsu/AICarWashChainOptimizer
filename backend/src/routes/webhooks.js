...
const ALLOWED_EVENTS = [
  'wash.completed', 'membership.signup', 'membership.cancelled',
  'equipment.maintenance_due', 'chemical.low', 'staffing.shift_open',
  'revenue.target_missed', 'feedback.received', 'weather.adverse'
];
...
async function ensureTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS webhooks (
      ...
      user_id INTEGER,
      url TEXT NOT NULL,
      events TEXT[] NOT NULL DEFAULT ARRAY['wash.completed']::text[],
      secret VARCHAR(255),
...
router.post('/', async (req, res) => {
  try {
    ...
    const incoming = Array.isArray(events) && events.length > 0 ? events : ['wash.completed'];
...
router.post('/:id/test', async (req, res) => {
  try {
    ...
    const payload = {
      ...
      data: { message: 'Test webhook from AICarWashChainOptimizer' }
    };
